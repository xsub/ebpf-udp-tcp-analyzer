...
class BpftoolMapReader:
    ...
    def refresh_map_id(self) -> int:
        ...
        # Pick the map BY NAME. `map_ids[0]` happened to work while the program owned
        # exactly one map; adding the drop-reason array turned it into a coin flip, and
        # losing that flip is SILENT: the counters map's 20-byte key is parsed with
        # '<BBHH2xIII', a 4-byte array key is not, so the reader produced nothing at all
        # while the kernel counted 126 Mb/s. `self.map_name` already existed and was
        # simply never consulted here.
        #
        # The kernel truncates map names to BPF_OBJ_NAME_LEN-1 = 15 chars, so
        # 'udp_ingress_counters' shows up as 'udp_ingress_cou'. Compare truncated.
